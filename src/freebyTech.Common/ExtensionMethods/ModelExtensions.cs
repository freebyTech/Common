...
namespace freebyTech.Common.ExtensionMethods
{
  public static class ModelExtensions
  {
    ...
    {
      var entityTypes = modelBuilder.Model.GetEntityTypes().Where(t => typeof(IEditableModel).IsAssignableFrom(t.ClrType));
      foreach (var entityType in entityTypes)
      {
        var configurationType = typeof(EditableModelConfiguration<>).MakeGenericType(entityType.ClrType);
        modelBuilder.ApplyConfiguration((dynamic)Activator.CreateInstance(configurationType));
      }
    }
